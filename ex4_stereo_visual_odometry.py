...
class VisualOdometry():
    ...
    def calculate_right_qs(self, q1, q2, disp1, disp2, min_disp=0.0, max_disp=100.0):
        ...
        """
        def get_indices(q, disp):
            q_idx = q.astype(int)
            disp = disp.T[q_idx[:, 0], q_idx[:, 1]]
            return disp, np.where(np.logical_and(min_disp < disp, disp < max_disp), True, False)

        disp1, mask1 = get_indices(q1, disp1)
        disp2, mask2 = get_indices(q2, disp2)
    ...
    def estimate_pose(self, q1, q2, Q1, Q2, max_iter=100):
        ...
        """
        early_termination_thresh = 5
        min_error = float('inf')
        early_termination = 0

        for _ in range(max_iter):
            sample_idx = np.random.choice(range(q1.shape[0]), 6)
            sample_q1, sample_q2 = q1[sample_idx], q2[sample_idx]
            sample_Q1, sample_Q2 = Q1[sample_idx], Q2[sample_idx]
            ...
            if early_termination == early_termination_thresh:
                break

        r = out_pose[:3]
        R, _ = cv2.Rodrigues(r)
        t = out_pose[3:]
        transf_matrix = self._form_transf(R, t)
