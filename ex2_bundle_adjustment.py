...
def objective(params, n_cams, n_Qs, cam_idxs, Q_idxs, qs):
    ...
    # Project 3D points into the image planes
...
def bundle_adjustment(cam_params, Qs, cam_idxs, Q_idxs, qs):
    ...
    """
    # Stack the camera params and the 3D points
    ...
    residual_solution = res.fun
    solution = res.x
    return residual_init, residual_solution, solution
...
def sparsity_matrix(n_cams, n_Qs, cam_idxs, Q_idxs):
    ...
    # Sparsity from camera params
    for s in range(9):
        sparse_mat[2 * i, cam_idxs * 9 + s] = 1
        sparse_mat[2 * i + 1, cam_idxs * 9 + s] = 1

    # print('Sparse mat from camera params:', sparse_mat)
...
def bundle_adjustment_with_sparsity(cam_params, Qs, cam_idxs, Q_idxs, qs, sparse_mat):
    ...
    #     t = cam_params[3:6]
    #     transformations.append(np.column((R, t)))

    bundle_adjustment(cam_params, Qs, cam_idxs, Q_idxs, qs)


def main():
    data_file = "../data/bundle_data.txt"
